...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.type === "TO_CHATGPT") {
        ...
        console.log("🖼️ [BACKGROUND] Image received. Focusing Studio...");
        relayToStudio(request);
        setTimeout(() => {
            focusStudio();
            setTimeout(() => {
    ...
    try {
        ...
        await chrome.scripting.executeScript({
            ...
            func: (text) => {
                ...
                // Add newline before prompt if image was inserted
        ...
        console.error("Grok Cycle Error:", e);
...
async function executeLiteralCycle(promptText, assets, assetIds, prevFrame) {
    ...
    try {
        ...
        if (!geminiTab) { report("❌ Gemini не найден."); return; }
                ...
                func: async (imgs) => {
                    ...
                    for (const b64 of imgs) {
                        try {
                            ...
                            dt.items.add(file);
                            ed.dispatchEvent(new ClipboardEvent('paste', { clipboardData: dt, bubbles: true }));
                            await new Promise(r => setTimeout(r, 1200)); // Wait for Gemini to process upload
        ...
        await chrome.scripting.executeScript({
            ...
            func: (text) => {
                ...
                // Use a safer way to append text if images are already there
        ...
        // 6. TELEGRAM VERSION LOGIC: Mouseenter + Copy Button + Fetch
        report("📸 КОПИРУЮ (Telegram Logic)...");
        await chrome.scripting.executeScript({
            target: { tabId: geminiTab.id },
            func: async () => {
                const sleep = (ms) => new Promise(r => setTimeout(r, ms));
                ...
                if (frames.length > 0) {
                    ...
                    // Look for Copy button in parent hierarchy
                    let p = img;
                    ...
                    // Fetch source as fallback/direct send
                    try {
